...
public class DanmakuCollider : MonoBehaviour {
  ...
  internal static unsafe void TestPoolCollisions(DanmakuPool pool) {
    ...
    for (var i = 0; i < count; i++) {
      ...
      var hitResults = Physics2D.CircleCastAll(oldPosition, pool.ColliderRadius, direction, distance, layerMask);
      if (hitResults.Length <= 0) continue;
      var danmaku = new Danmaku(pool, i);
      for (var j = 0; j < hitResults.Length; j++) {
        var collider = hitResults[j].collider;
